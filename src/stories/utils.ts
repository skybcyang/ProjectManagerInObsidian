...
type EntityManagerLike = {
  ...
  getEntityPath(type: 'version' | 'project' | 'feature', id: string): Promise<string | null>;
};
...
export function createStoryEntityManager(
  ...
  return {
    ...
    getOwners: () => {
      ...
      [...versions, ...projects, ...features].forEach(e => {
...
class StoryActionService {
  ...
  async openEntity(type: 'version' | 'project' | 'feature', id: string): Promise<void> {
