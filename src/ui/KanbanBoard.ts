...
import { FEATURE_STATUSES, getStatusLabel } from '../constants';
...
export class KanbanBoard {
  ...
  private async renderAll(
    ...
    for (const status of statuses) {
      const column = this.createColumn(board, status, columns[status] ?? [], isCompact);
  ...
  private async renderByVersion(
    ...
    for (const [versionId, versionFeatures] of versionGroups) {
      ...
      for (const status of statuses) {
        const column = this.createColumn(board, status, columns[status] ?? [], true);
        board.appendChild(column);
      }
    }
  }
    ...
    if (cardRenderer) {
      for (const feature of features) {
        ...
        const card = isCompact
          ? (cardRenderer as any).renderCompact?.(feature, onClick) ?? cardRenderer.render(feature, onClick)
          : cardRenderer.render(feature, onClick);
   ...
   * 按状态分组
   */
  private groupByStatus(features: Feature[]): Record<FeatureStatus, Feature[]> {
    const result: Record<FeatureStatus, Feature[]> = {
      backlog: [],
      todo: [],
      'in-progress': [],
      testing: [],
